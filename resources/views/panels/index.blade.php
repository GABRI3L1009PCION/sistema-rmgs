...
@section('content')
    @php
        $panelDetails = $panelModels->mapWithKeys(fn ($panel) => [$panel->id => [
            'id' => $panel->id, 'brand' => $panel->brand, 'model' => $panel->model,
            'power_kw' => (float) $panel->nominal_power_kw, 'status' => $panel->status,
            'quantity' => $panel->farmPanels->sum('quantity'),
            'farms' => $panel->farmPanels->map(fn ($item) => ['name' => $item->solarFarm->name, 'quantity' => $item->quantity])->values(),
        ]]);
    @endphp
    <style>
        ...
        .panel-state.inactive { color: var(--muted); }.panel-state.inactive::before { background: #8ca1bd; }
        .row-menu { border: 0; background: transparent; color: var(--ink); cursor: pointer; font-weight: 900; }
        ...
        .detail-head h3 { font-size: 1rem; line-height: 1.15; }.detail-head p { margin-top: 4px; font-size: .68rem; color: var(--muted); }
        ...
        .detail-body { min-height: 0; padding-top: 10px; }.spec-grid { display: grid; grid-template-columns: repeat(2,1fr); gap: 10px 16px; padding-bottom: 12px; border-bottom: 1px solid var(--line); }
        .spec { display: grid; grid-template-columns: 20px 1fr; gap: 7px; font-size: .68rem; color: var(--muted); }.spec svg { width: 16px; color: #5976aa; }
        .installations-title { margin: 12px 0 8px; font-size: .76rem; }.installation-list { display: grid; gap: 6px; }
        .installation-row { display: flex; justify-content: space-between; gap: 10px; padding: 7px 8px; border-radius: 6px; background: #f7fafe; font-size: .66rem; }.installation-row strong { white-space: nowrap; }
    ...
    <div class="panels-screen">
        <section class="panels-hero"><div><p class="eyebrow">RMGS - Registro y Monitoreo de Generacion Solar Guatemala</p><h1>Paneles solares</h1><p class="subtitle">Tecnologia que impulsa un Guatemala mas limpio y sostenible.</p></div><div class="hero-note"><i data-lucide="map-pin"></i><span>Guatemala, un pais con mas energia limpia</span></div></section>
        ...
        <section class="panels-content">
            <article class="card panel-list"><header class="panel-list-title"><h2><i data-lucide="grid-2x2"></i>Modelos de paneles solares</h2><span>Mostrando <strong id="panel-visible-count">{{ $panelModels->sum(fn($panel)=>$panel->farmPanels->count()) }}</strong> instalaciones</span></header><div class="panel-table-wrap"><table class="panel-table"><thead><tr><th style="width:29%">Modelo</th><th style="width:13%">Potencia (Wp)</th><th style="width:17%">Cantidad instalada</th><th style="width:22%">Granja</th><th style="width:13%">Estado</th><th style="width:6%">Acciones</th></tr></thead><tbody>@foreach($panelModels as $panel)@forelse($panel->farmPanels as $installation)<tr class="panel-row" data-panel-id="{{ $panel->id }}" data-brand="{{ $panel->brand }}" data-status="{{ $panel->status }}" data-farm="{{ $installation->solarFarm->name }}" data-search="{{ strtolower($panel->brand.' '.$panel->model) }}"><td><span class="panel-name"><span class="panel-mini"></span><strong>{{ $panel->brand }} {{ $panel->model }}</strong></span></td><td>{{ number_format($panel->nominal_power_kw*1000) }}</td><td>{{ number_format($installation->quantity) }}</td><td>{{ $installation->solarFarm->name }}</td><td><span class="panel-state {{ $panel->status }}">{{ $panel->status==='active'?'Activo':'Inactivo' }}</span></td><td><button class="row-menu" type="button">•••</button></td></tr>@empty<tr class="panel-row" data-panel-id="{{ $panel->id }}" data-brand="{{ $panel->brand }}" data-status="{{ $panel->status }}" data-farm="" data-search="{{ strtolower($panel->brand.' '.$panel->model) }}"><td><span class="panel-name"><span class="panel-mini"></span><strong>{{ $panel->brand }} {{ $panel->model }}</strong></span></td><td>{{ number_format($panel->nominal_power_kw*1000) }}</td><td>0</td><td>Sin asignar</td><td><span class="panel-state {{ $panel->status }}">{{ $panel->status==='active'?'Activo':'Inactivo' }}</span></td><td><button class="row-menu" type="button">•••</button></td></tr>@endforelse @endforeach</tbody></table></div></article>
            <aside class="card panel-detail"><header class="panel-detail-title"><h2><i data-lucide="grid-2x2"></i>Detalles del panel</h2></header><div class="detail-head"><div class="panel-visual"></div><div><h3 id="detail-model">Selecciona un panel</h3><p id="detail-brand">Modelo registrado</p><p>Panel solar fotovoltaico de alta eficiencia.</p></div><span class="detail-badge" id="detail-status">Activo</span></div><div class="detail-body"><div class="spec-grid"><div class="spec"><i data-lucide="zap"></i><span>Potencia nominal<br><strong id="detail-power">0 Wp</strong></span></div><div class="spec"><i data-lucide="layers-3"></i><span>Tecnologia<br><strong>Monocristalino</strong></span></div><div class="spec"><i data-lucide="box"></i><span>Tipo<br><strong>Modulo fotovoltaico</strong></span></div><div class="spec"><i data-lucide="activity"></i><span>Estado<br><strong id="detail-state">Activo</strong></span></div></div><h3 class="installations-title">Instalacion</h3><div class="installation-list" id="installation-list"></div></div></aside>
        </section>
    </div>
    <script>
        const panelDetails=@json($panelDetails),panelRows=[...document.querySelectorAll('.panel-row')],brandFilter=document.getElementById('brand-filter'),statusFilter=document.getElementById('panel-status-filter'),farmFilter=document.getElementById('panel-farm-filter'),panelSearch=document.getElementById('panel-search');
        function showPanel(id){const panel=panelDetails[id];if(!panel)return;document.getElementById('detail-model').textContent=`${panel.brand} ${panel.model}`;document.getElementById('detail-brand').textContent=panel.brand;document.getElementById('detail-power').textContent=`${Math.round(panel.power_kw*1000)} Wp`;document.getElementById('detail-status').textContent=panel.status==='active'?'Activo':'Inactivo';document.getElementById('detail-state').textContent=panel.status==='active'?'Activo':'Inactivo';document.getElementById('installation-list').innerHTML=panel.farms.length?panel.farms.map(farm=>`<div class="installation-row"><span>${farm.name}</span><strong>${Number(farm.quantity).toLocaleString()} paneles</strong></div>`).join(''):'<div class="installation-row"><span>Sin instalaciones asociadas</span></div>';panelRows.forEach(row=>row.classList.toggle('selected',Number(row.dataset.panelId)===Number(id)))}
        function filterPanels(){const query=panelSearch.value.trim().toLowerCase();let visible=0;panelRows.forEach(row=>{const show=(!brandFilter.value||row.dataset.brand===brandFilter.value)&&(!statusFilter.value||row.dataset.status===statusFilter.value)&&(!farmFilter.value||row.dataset.farm===farmFilter.value)&&(!query||row.dataset.search.includes(query));row.hidden=!show;if(show)visible++});document.getElementById('panel-visible-count').textContent=visible}
        panelRows.forEach(row=>row.addEventListener('click',()=>showPanel(row.dataset.panelId)));[brandFilter,statusFilter,farmFilter,panelSearch].forEach(control=>control.addEventListener('input',filterPanels));if(panelRows.length)showPanel(panelRows[0].dataset.panelId);
    </script>
@endsection
